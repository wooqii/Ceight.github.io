...
contract Owned {
    ...
    contract Members is Owned {
        // (3) 상태 변수 선언
        address public coin; // 토큰(가상화폐) 주소
        ...
        // (8) 회원 등급 내용 변경
        function editSatus(uint256 _index, string _name, uint256 _times, uint256 _sum, int8 _rate) public onlyOwner {
        ...
        function updateHistory(address _member, uint256 _value) public onlyCoin {
            ...
            uint256 index;
            int8 tmprate;
            for (var uint i = 0; i < status.length; i++) {
    ...
    // (11) 회원관리 기능이 구현된 가상화폐
contract CeightToken is Owned{
        // 상태변수 선언
